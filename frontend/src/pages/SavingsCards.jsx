...
import { Landmark, Plus, Edit, Trash2, X, Save } from 'lucide-react';
import { savingsCardsApi, participantsApi } from '../services/api';
...
export default function SavingsCards() {
  ...
  const fetchSavingsCards = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await savingsCardsApi.getAll(false);
      setCards(data);
    } catch (err) {
      console.error('Error fetching savings cards:', err);
  ...
  const handleSaveCard = async () => {
    try {
      ...
      if (editingCard) {
        await savingsCardsApi.update(editingCard.id, payload);
      } else {
        await savingsCardsApi.create(payload);
      }

      setShowModal(false);
      setEditingCard(null);
      await fetchSavingsCards();
    } catch (err) {
      console.error('Savings card error:', err);
      alert('Error al guardar la tarjeta de ahorro: ' + (err.message || JSON.stringify(err)));
    }
  };
  ...
  const handleDeleteCard = async (card) => {
    if (window.confirm(`¿Seguro que quieres eliminar ${card.name}?`)) {
      try {
        await savingsCardsApi.delete(card.id);
        await fetchSavingsCards();
      } catch (err) {
        alert('Error al eliminar la tarjeta de ahorro: ' + err.message);
      }
    }
  };
    ...
    return (
      <div className="page">
        <div style={{ textAlign: 'center', padding: '4rem' }}>
          <p style={{ fontSize: '1.2rem', color: 'var(--text-secondary)' }}>Cargando tarjetas de ahorro...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h1>Tarjetas de ahorro</h1>
          <p className="text-secondary">Registra tus cuentas de ahorro para gestionar transferencias</p>
          {error && (
            <div style={{
              marginTop: '0.5rem',
              padding: '0.5rem',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
          ...
          <div
            ...
            style={{
              background: card.active
                ? 'linear-gradient(135deg, #5B6FA6, #3D4F8A)'
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
              <div>
                ...
                <div style={{ fontSize: '1.25rem', fontWeight: 600, letterSpacing: '0.05em' }}>
                  {card.last_four_digits ? `•••• ${card.last_four_digits}` : 'Cuenta de ahorro'}
                </div>
              </div>
              <Landmark size={32} style={{ opacity: 0.8 }} />
            </div>
        ...
        {filteredCards.length === 0 && (
          <div className="card" style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '3rem' }}>
            <Landmark size={64} style={{ margin: '0 auto 1rem', opacity: 0.3 }} />
            <h3 style={{ marginBottom: '0.5rem' }}>Aún no hay tarjetas de ahorro</h3>
            <p className="text-secondary">Registra tu primera cuenta de ahorro para usarla en transferencias</p>
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <h2 style={{ margin: 0 }}>{editingCard ? 'Editar cuenta de ahorro' : 'Nueva cuenta de ahorro'}</h2>
              <button onClick={() => setShowModal(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '0.5rem', display: 'flex' }}>
                <X size={24} />
              </button>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                ...
                <input
                  ...
                  placeholder="Ej.: Ahorros BCP, Cuenta AFP"
                  className="input"
                />
              </div>
              ...
              <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
                <button className="btn btn-primary" onClick={handleSaveCard} style={{ flex: 1 }}>
                  <Save size={16} />
                  Guardar
                </button>
                <button className="btn" onClick={() => setShowModal(false)} style={{ flex: 1 }}>
                  <X size={16} />
                  Cancelar
                </button>
              </div>
